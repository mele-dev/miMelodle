...
RETURNING status, (SELECT username
                   FROM target) AS "targetUsername!";
...
WITH "newestGame" AS (SELECT *
     ...
     "canCreateGame" AS (SELECT CASE
                                    ...
                                    WHEN (SELECT "createdAt"::DATE
                                          FROM "newestGame") != CURRENT_DATE THEN TRUE
                                    ELSE FALSE
                                    END AS "canCreate"),
     "insertGame"
         AS ( INSERT INTO "guessSongGames" ("userId", "createdAt", "spotifyTrackId", "snippet") SELECT :selfId!, NOW(), :spotifyTrackId!, :snippet
                                                                                                WHERE EXISTS (SELECT 1
                                                                                                              FROM "canCreateGame"
                                                                                                              WHERE "canCreate" = TRUE) RETURNING id)
